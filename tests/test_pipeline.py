...
def test_pipeline(tmp_path, caplog):
    ...
    assert f"Reading file {source_filepath}" in caplog.text
    assert f"Writing into file {storage_filepath}" in caplog.text
    assert "Pipeline finished" in caplog.text
